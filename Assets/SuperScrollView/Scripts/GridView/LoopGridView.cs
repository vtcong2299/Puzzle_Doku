...
namespace SuperScrollView
{
    ...
    public class LoopGridView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public bool IsInitialized => mListViewInited;
        ...
        */
        ...
        public Vector2 GetItemPos(int row, int column)
        {
            ...
            else if (ArrangeType == GridItemArrangeType.TopCenter)
            {
                float centerX = GetCenterOffset(column) * mItemSizeWithPadding.x;
                return new Vector2(centerX, -y);
            }
            else if (ArrangeType == GridItemArrangeType.BotCenter)
            {
                float centerX = GetCenterOffset(column) * mItemSizeWithPadding.x;
                return new Vector2(centerX, y);
            }
            else if (ArrangeType == GridItemArrangeType.MidleLeft)
            {
                float centerY = GetCenterOffset(row) * mItemSizeWithPadding.y;
                return new Vector2(x, -centerY);
            }
            else if (ArrangeType == GridItemArrangeType.MidleCenter)
            {
                float centerX = GetCenterOffset(column) * mItemSizeWithPadding.x;
                float centerY = GetCenterOffset(row) * mItemSizeWithPadding.y;
        ...
        void AdjustViewPortPivot()
        {
            ...
            {
                rtf.pivot = new Vector2(0.5f, 1);
            }
            else if (ArrangeType == GridItemArrangeType.BotCenter)
            {
                rtf.pivot = new Vector2(0.5f, 0);
            }
            else if (ArrangeType == GridItemArrangeType.MidleLeft)
            {
        ...
        void AdjustContainerAnchorAndPivot()
        {
            ...
            {
                rtf.anchorMin = new Vector2(0, 0.5f);
                rtf.anchorMax = new Vector2(0, 0.5f);
                rtf.pivot = new Vector2(0, 0.5f);
            }
            else if (ArrangeType == GridItemArrangeType.MidleCenter)
            {
        ...
        Vector2 GetContainerVaildPos(float curX, float curY)
        {
            ...
            else if (mArrangeType == GridItemArrangeType.TopCenter)
            {
                curX = Mathf.Clamp(curX, -maxCanMoveX * 0.5f, maxCanMoveX * 0.5f);
                curY = Mathf.Clamp(curY, 0, maxCanMoveY);
            }
            else if (mArrangeType == GridItemArrangeType.BotCenter)
            {
                curX = Mathf.Clamp(curX, -maxCanMoveX * 0.5f, maxCanMoveX * 0.5f);
                curY = Mathf.Clamp(curY, -maxCanMoveY, 0);
            }
            else if (mArrangeType == GridItemArrangeType.MidleLeft)
            {
                curX = Mathf.Clamp(curX, -maxCanMoveX, 0);
                curY = Mathf.Clamp(curY, -maxCanMoveY * 0.5f, maxCanMoveY * 0.5f);
            }
            else if (mArrangeType == GridItemArrangeType.MidleCenter)
            {
                curX = Mathf.Clamp(curX, -maxCanMoveX * 0.5f, maxCanMoveX * 0.5f);
                curY = Mathf.Clamp(curY, -maxCanMoveY * 0.5f, maxCanMoveY * 0.5f);
            }
        ...
        Vector2 GetItemSnapPivotLocalPos(int row, int column)
        {
            ...
            if (mArrangeType == GridItemArrangeType.TopCenter)
            {
                return new Vector2(
                    absPos.x,
                    absPos.y - mItemSize.y * (1 - mItemSnapPivot.y)
                );
            }
            else if (mArrangeType == GridItemArrangeType.BotCenter)
            {
                return new Vector2(
                    absPos.x,
                    absPos.y + mItemSize.y * mItemSnapPivot.y
                );
            }
            else if (mArrangeType == GridItemArrangeType.MidleLeft)
            {
                return new Vector2(
                    absPos.x + mItemSize.x * mItemSnapPivot.x,
                    absPos.y
                );
            }
            else if (mArrangeType == GridItemArrangeType.MidleCenter)
            {
                return new Vector2(
                    absPos.x + mItemSize.x * (mItemSnapPivot.x - 0.5f),
                    absPos.y + mItemSize.y * (mItemSnapPivot.y - 0.5f)
                );
            }
        ...
        Vector2 GetViewPortSnapPivotLocalPos(Vector2 pos)
        {
            ...
            {
                pivotLocalPosX = -pos.x + ViewPortWidth * (mViewPortSnapPivot.x - 0.5f);
                pivotLocalPosY = -pos.y - ViewPortHeight * (1 - mViewPortSnapPivot.y);
            }
            else if (mArrangeType == GridItemArrangeType.BotCenter)
            {
            ...
            {
                pivotLocalPosX = -pos.x + ViewPortWidth * mViewPortSnapPivot.x;
                pivotLocalPosY = -pos.y + ViewPortHeight * (mViewPortSnapPivot.y - 0.5f);
            }
            else if (mArrangeType == GridItemArrangeType.MidleCenter)
            {
                pivotLocalPosX = -pos.x + ViewPortWidth * (mViewPortSnapPivot.x - 0.5f);
        ...
        public void MovePanelToItemByIndexEnd(int itemIndex, float duration = 0f)
        {
            if (!mListViewInited || itemIndex < 0 || itemIndex >= mItemTotalCount)
                return;

            var rowColumn = GetRowColumnByItemIndex(itemIndex);
            MovePanelToItemByRowColumnEnd(rowColumn.mRow, rowColumn.mColumn, duration);
        }

        public void MovePanelToItemByRowColumnEnd(int row, int column, float duration = 0f)
        {
            if (!mListViewInited)
                return;

            mScrollRect.StopMovement();
            DOTween.Kill(mContainerTrans);
            ...
            switch (mArrangeType)
            {
                ...
                case GridItemArrangeType.TopCenter:
                    targetPos = new Vector2(
                        mContainerTrans.anchoredPosition.x,
                        -itemPos.y - mItemSize.y + ViewPortHeight);
                    break;

                case GridItemArrangeType.BottomLeftToTopRight:
                case GridItemArrangeType.BottomRightToTopLeft:
                case GridItemArrangeType.BotCenter:
                    targetPos = new Vector2(
                        mContainerTrans.anchoredPosition.x,
                        -itemPos.y - ViewPortHeight + mItemSize.y);
                    break;

                case GridItemArrangeType.MidleLeft:
                    targetPos = new Vector2(
                        -itemPos.x - mItemSize.x + ViewPortWidth,
                        mContainerTrans.anchoredPosition.y);
                    break;

                case GridItemArrangeType.MidleCenter:
                    targetPos = new Vector2(
                        -itemPos.x,
                        -itemPos.y);
                    break;
            ...
            targetPos = GetContainerVaildPos(targetPos.x, targetPos.y);

            if (duration <= 0f)
            {
                mContainerTrans.anchoredPosition = targetPos;
                ForceToCheckContentPos();
                return;
            }
            ...
            mContainerTrans
                ...
                .OnComplete(ForceToCheckContentPos);
        }
    }
}